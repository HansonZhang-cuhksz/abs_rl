import torch
from torch_train import DQN, TransformerDQN  # Replace with your model class

# Check if CUDA is available and set the device
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

# Load the trained model
checkpoint = torch.load('dqn_model_B.pth', weights_only=True)
model = TransformerDQN(7, 3)  # Replace with your model class
...
def predict(state, available_bitrates):
    ...
    state_tensor = torch.tensor(state, dtype=torch.float32).unsqueeze(0).to(device)  # Add batch dimension and move to GPU
